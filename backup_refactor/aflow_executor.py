...
class AFlowExecutor:
    ...
    def __init__(
        ...
        self.validator = WorkflowValidator()  # 统一验证器（支持reactive patching + TASK_PROMPT提取）
    ...
    async def execute_workflow(
        ...
        **kwargs
        ...
        # 1. 验证工作流代码
        # 使用WorkflowValidator进行reactive patching验证和修复
    ...
    def _create_workflow_class(self, workflow_code: str, problem_type: str):
        ...
        # 1. 提取TASK_PROMPT（可选）
        task_prompt = self.validator.extract_task_prompt(workflow_code)
        if task_prompt:
